...
## According to the ATAC-Seq paper To shifting
my $plusShift = 4;
my $minusShift = 5;
...
my $com1 = "samtools view -F 4 $inbam |perl -lane 'unless(\$F[1] \& 0x0010) {print join \"\\t\",(\$F[2],\$F[3]-1+$plusShift,\$F[3]+$plusShift)} else{print join \"\\t\",(\$F[2],\$F[3]-1-$minusShift,\$F[3]-$minusShift)}' |sort -k1,1 -k2,2n | bedItemOverlapCount $genome -chromSize=$chromsize stdin |sort -k1,1 -k2,2n > $outbedgraph";
