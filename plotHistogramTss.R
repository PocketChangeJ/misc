...
dat<-read.delim(file = "tss_4k_hist10_h9_D7unsort_D7CD44hi_D7CD44lo_histogram.txt",row.names=1)

idx<-seq(from = 1,by = 3,length.out = 8)
d<-dat[,idx]

col.nms<-gsub(pattern = ".*?tags\\.(.*?)\\.\\.Coverage",replacement = "\\1",x = colnames(d))
...
legend("topright",legend = col.nms,lty=1:8,col=cols,lwd=2,bty="n")
title("ATAC-Seq (miSeq)")
dev.off()


## Fold change to its -2000bp -1800bp signals
